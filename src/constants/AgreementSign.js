export const AgreementSign = `<br><br> <p class="long-text " style="color: #000000;
        font-size: 19px;
        line-height: 24px;
        font-family: Arial-Regular;"><span style="font-weight: 800;
        font-family: Arial-bold;"> IN WITNESS WHEREOF,</span> the undersigned have executed and delivered this Agreement.
    </p> <table style="width:100%">
        <tr style="height: 30px; vertical-align: top">
            <td style="width:19%">Company:</td>
            <td width:50% style="font-weight: 700; "width:31%;">Buy Box Experts</td>
            <td > &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                <td style="width:19%">Company:</td>
                <td style="font-weight: 700; "width:31%;">CUSTOMER_NAME</td>
        </tr>
         <tr style="height: 30px; vertical-align: top">
            <td>Signature:</td>
            <td style="font-weight: 700;" > THAD_SIGN</td>
            <td></td>
            <td>Signature:</td>
            <td style="font-weight: 700;"></td>
        </tr>
        <tr style="height: 30px; vertical-align: top">
            <td width:50%>
                Printed Name:
            </td>
            <td width:50% style="font-weight: 700;" >
                Thaddaeus Hay
            </td>
            <td></td>
            <td width:50%>
                Printed Name:
            </td width:50%>
            <td style="font-weight: 700;">
                PRINTED_NAME
            </td>
        </tr>
        <tr style="height: 30px; vertical-align: top">
            <td>Title:</td>
            <td style="font-weight: 700;">
            Chief Revenue Officer
            </td>
            <td></td>
            <td>Title:</td>
            <td style="font-weight: 700;">
              CUSTOMER_ROLE
            </td>
        </tr>
        <tr style="height: 30px; vertical-align: top">
            <td>Date:</td>
            <td style="font-weight: 700;">BBE_DATE</td>
            <td></td>
            <td>Date:</td>
            <td style="font-weight: 700;"></td>
        </tr>
        <tr style="height: 30px; vertical-align: top">
            <td>Address:</td>
            <td style="font-weight: 700;">1172 W 700 N #200 Lindon, UT 84042
</td>
            <td></td>
            <td>Address:</td>
            <td style="font-weight: 700;">CUSTOMER_ADDRESS</td>
        </tr>
    </table>`;

export const AddendumSign = `<br><br> <p class="long-text " style="color: #000000;
        font-size: 19px;
        line-height: 24px;
        font-family: Arial-Regular;"><span style="font-weight: 800;
        font-family: Arial-bold;"> IN WITNESS WHEREOF,</span> the undersigned have executed and delivered this Agreement.
    </p>
    <table style="width:100%"><tr style="height: 30px; vertical-align: top">
            <td  style="width:19%">Company:</td>
            <td style="font-weight: 700; "width:31%;">Buy Box Experts</td>
            <td> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;</td>
                <td   style="width:19%">Company:</td>
                <td  style="font-weight: 700;"width:31%">CUSTOMER_NAME</td>
        </tr>
         <tr style="height: 30px; vertical-align: top">
            <td>Signature:</td>
            <td style="font-weight: 700;" >THAD_SIGN</td>
            <td></td>
            <td>Signature:</td>
            <td style="font-weight: 700;"></td>
        </tr>
        <tr style="height: 30px; vertical-align: top">
            <td>
                Printed Name:
            </td>
            <td style="font-weight: 700;" >
                Thaddaeus Hay
            </td>
            <td></td>
            <td>
                Printed Name:
            </td>
            <td style="font-weight: 700;">
               PRINTED_NAME
            </td>
        </tr>
        
        <tr style="height: px; vertical-align: top">
            <td>Date:</td>
            <td style="font-weight: 700;">BBE_DATE</td>
            <td></td>
            <td>Date:</td>
            <td style="font-weight: 700;"></td>
        </tr>
        
    </table>`;
